...
//   a. Check if the array is full by checking if next[0] is -1. If it is, return an error message indicating that the stack is full.
//   b. Set the value of next[0] to top[i].
//   c. Set the value of top[i] to 0.
//   d. Set the value of next[top[i]] to the new element’s index.
//   e. Increment the value of top[i] by the block size.
// 5. To pop an element from the i-th stack, do the following:
//  a. Check if the array is empty by checking if top[i] is -1. If it is, return an error message indicating that the stack is empty.
//  b. Decrement the value of top[i] by the block size.
//  c. Set the value of next[top[i]] to -1.
//  d. Return the element at the index top[i] + block size – 1.
